...
func setup(poke_start : Pokemon, poke_ID_end : int):
	...
	start_sprite.material.shader = load("res://Utilities/Battle/WhiteFade.gdshader")
	...
	end_sprite.material.shader = load("res://Utilities/Battle/WhiteFade.gdshader")
...
func run():
	...
	Global.game.play_dialogue(tr("What?\n{1} is evolving!").replace("{1}", tr(start_name)))
	...
	Global.game.play_dialogue(tr("Congratulations!\nYour {1} evolved into {2}!").replace("{1}", tr(start_name)).replace("{2}", tr(end_name)))
	await Global.game.event_dialogue_end
	emit_signal("close")
	effect_enable = false
