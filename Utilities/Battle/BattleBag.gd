		...
		if event.is_action_pressed("x"):
			match stage:
				...
				2:
					...
					bag_tween.tween_property($BagMenu/Pages, "position", Vector2(512,0), 0.25).set_trans(Tween.TRANS_SINE).set_ease(Tween.EASE_OUT)
				...
				1:
					...
					match selection:
						MEDICINE,POKEBALLS,BERRIES,BATTLE_ITEMS:
							...
							# Godot 4: Create tween programmatically
							var bag_tween = create_tween()
							bag_tween.tween_property($BagMenu/Pages, "position", Vector2(0,0), 0.25).from(Vector2(512,0)).set_trans(Tween.TRANS_SINE).set_ease(Tween.EASE_OUT)
							selection = S1
						LAST_ITEM:
							pass
						BACK:
...
func left_page(select):
	...
	# Godot 4: Create tween programmatically
	var bag_tween = create_tween()
	bag_tween.tween_property($BagMenu/Pages, "position", $BagMenu/Pages.position + Vector2(512,0), 0.25).set_trans(Tween.TRANS_SINE).set_ease(Tween.EASE_OUT)
	await $BagMenu/Tween.tween_completed
