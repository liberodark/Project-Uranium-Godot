extends Node2D # gen_map.py Map122

var map_name = "Nuclear Plant Epsilon(1)"
...
func interaction(check_pos : Vector2, _direction):
	if check_pos == Vector2(1264, 1200):
		Global.game.play_dialogue("MAP122_NPC_1")
	if check_pos == Vector2(1520, 1040):
		Global.game.play_dialogue("MAP122_NPC_2")
	if check_pos == Vector2(1520, 1296):
		Global.game.play_dialogue("MAP122_NPC_3")
	if check_pos == Vector2(1776, 1104):
		Global.game.play_dialogue("MAP122_NPC_4")
	if check_pos == Vector2(1328, 1072):
		Global.game.play_dialogue("MAP122_NPC_5")
	if check_pos == Vector2(1264, 1392):
		Global.game.play_dialogue("MAP122_NPC_6")
	if check_pos == Vector2(1584, 1392):
		Global.game.play_dialogue("MAP122_NPC_7")
	return null

var _trainer_data = {
	"Trainer1": {"pre": "MAP122_TRAINER_1", "defeat": "MAP122_TRAINER_1_DEFEAT", "portrait": "Graphics/Characters/trainer090.png"},
	"Trainer2": {"pre": "MAP122_TRAINER_2", "defeat": "MAP122_TRAINER_2_DEFEAT", "portrait": "Graphics/Characters/trainer090.png"},
	"Trainer3": {"pre": "MAP122_TRAINER_3", "defeat": "MAP122_TRAINER_3_DEFEAT", "portrait": "Graphics/Characters/trainer090.png"}
}

func _ready():
	Global.game.player.connect("trainer_battle", Callable(self, "trainer_battle"))
	for c in $NPC_Layer.get_children():
		if String(c.name).begins_with("Item") and Global.past_events.has("MAP122_ITEM_" + String(c.name).trim_prefix("Item") + "_TAKEN"):
			c.queue_free()
		elif String(c.name).begins_with("Trainer") and Global.past_events.has("MAP122_" + String(c.name).to_upper() + "_DEFEATED"):
			c.seeking = false
			c.defeated = true
...
func trainer_battle(npc_trainer):
	...
	if Global.game.battle.player_won:
		...
		Global.past_events.append("MAP122_" + String(npc_trainer.name).to_upper() + "_DEFEATED")
