extends Node2D # gen_map.py Map195

var map_name = "Championship Site (arena 2)"

var _trainer_data = {
	"Trainer1": {"pre": "MAP195_TRAINER_1", "defeat": "MAP195_TRAINER_1_DEFEAT", "portrait": "Graphics/Characters/trainer110.png"}
}

func _ready():
	Global.game.player.connect("trainer_battle", Callable(self, "trainer_battle"))
	for c in $NPC_Layer.get_children():
		if String(c.name).begins_with("Item") and Global.past_events.has("MAP195_ITEM_" + String(c.name).trim_prefix("Item") + "_TAKEN"):
			c.queue_free()
		elif String(c.name).begins_with("Trainer") and Global.past_events.has("MAP195_" + String(c.name).to_upper() + "_DEFEATED"):
			c.seeking = false
			c.defeated = true
...
func trainer_battle(npc_trainer):
	...
	if Global.game.battle.player_won:
		...
		Global.past_events.append("MAP195_" + String(npc_trainer.name).to_upper() + "_DEFEATED")
