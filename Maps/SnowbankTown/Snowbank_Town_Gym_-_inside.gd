extends Node2D # gen_map.py Map137

var map_name = "Snowbank Town(Gym - inside)"

func interaction(check_pos : Vector2, _direction):
	if check_pos == Vector2(2096, 1968):
		Global.game.play_dialogue("MAP137_SIGN_1")
	if check_pos == Vector2(2096, 1616):
		Global.game.play_dialogue("MAP137_SIGN_2")
	if check_pos == Vector2(1136, 912):
		Global.game.play_dialogue("MAP137_NPC_1")
	if check_pos == Vector2(1296, 48):
		Global.game.play_dialogue("MAP137_NPC_2")
	return null

var _trainer_data = {
	"Trainer1": {"pre": "MAP137_TRAINER_1", "defeat": "MAP137_TRAINER_1_DEFEAT", "portrait": "Graphics/Characters/trainer105.png"},
	"Trainer2": {"pre": "MAP137_TRAINER_2", "defeat": "MAP137_TRAINER_2_DEFEAT", "portrait": "Graphics/Characters/trainer106.png"},
	"Trainer3": {"pre": "MAP137_TRAINER_3", "defeat": "MAP137_TRAINER_3_DEFEAT", "portrait": "Graphics/Characters/trainer108.png"}
}

func _ready():
	Global.game.player.connect("trainer_battle", Callable(self, "trainer_battle"))
	for c in $NPC_Layer.get_children():
		if String(c.name).begins_with("Item") and Global.past_events.has("MAP137_ITEM_" + String(c.name).trim_prefix("Item") + "_TAKEN"):
			c.queue_free()
		elif String(c.name).begins_with("Trainer") and Global.past_events.has("MAP137_" + String(c.name).to_upper() + "_DEFEATED"):
			c.seeking = false
			c.defeated = true
...
func trainer_battle(npc_trainer):
	...
	if Global.game.battle.player_won:
		...
		Global.past_events.append("MAP137_" + String(npc_trainer.name).to_upper() + "_DEFEATED")
