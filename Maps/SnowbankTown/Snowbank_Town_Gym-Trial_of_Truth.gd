extends Node2D # gen_map.py Map206

var map_name = "Snowbank Town(Gym-Trial of Truth)"

func interaction(check_pos : Vector2, _direction):
	if check_pos == Vector2(784, 1200):
		Global.game.play_dialogue("MAP206_SIGN_1")
	return null

var _trainer_data = {
	"Trainer1": {"pre": "MAP206_TRAINER_1", "defeat": "MAP206_TRAINER_1_DEFEAT", "portrait": "Graphics/Characters/trainer109.png"}
}

func _ready():
	Global.game.player.connect("trainer_battle", Callable(self, "trainer_battle"))
	for c in $NPC_Layer.get_children():
		if String(c.name).begins_with("Item") and Global.past_events.has("MAP206_ITEM_" + String(c.name).trim_prefix("Item") + "_TAKEN"):
			c.queue_free()
		elif String(c.name).begins_with("Trainer") and Global.past_events.has("MAP206_" + String(c.name).to_upper() + "_DEFEATED"):
			c.seeking = false
			c.defeated = true
...
func trainer_battle(npc_trainer):
	...
	if Global.game.battle.player_won:
		...
		Global.past_events.append("MAP206_" + String(npc_trainer.name).to_upper() + "_DEFEATED")
