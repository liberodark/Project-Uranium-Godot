extends Node2D # gen_map.py Map141

var map_name = "Snowbank Town(Gym- Outside)"

func interaction(check_pos : Vector2, _direction):
	if check_pos == Vector2(2704, 560):
		Global.game.play_dialogue("MAP141_SIGN_1")
	if check_pos == Vector2(464, 368):
		Global.game.play_dialogue("MAP141_NPC_1")
	return null

var _trainer_data = {
	"Trainer1": {"pre": "MAP141_TRAINER_1", "defeat": "MAP141_TRAINER_1_DEFEAT", "portrait": "Graphics/Characters/trainer107.png"},
	"Trainer2": {"pre": "MAP141_TRAINER_2", "defeat": "MAP141_TRAINER_2_DEFEAT", "portrait": "Graphics/Characters/trainer109.png"},
	"Trainer3": {"pre": "MAP141_TRAINER_3", "defeat": "MAP141_TRAINER_3_DEFEAT", "portrait": "Graphics/Characters/trainer103.png"}
}

func _ready():
	Global.game.player.connect("trainer_battle", Callable(self, "trainer_battle"))
	for c in $NPC_Layer.get_children():
		if String(c.name).begins_with("Item") and Global.past_events.has("MAP141_ITEM_" + String(c.name).trim_prefix("Item") + "_TAKEN"):
			c.queue_free()
		elif String(c.name).begins_with("Trainer") and Global.past_events.has("MAP141_" + String(c.name).to_upper() + "_DEFEATED"):
			c.seeking = false
			c.defeated = true
...
func trainer_battle(npc_trainer):
	...
	if Global.game.battle.player_won:
		...
		Global.past_events.append("MAP141_" + String(npc_trainer.name).to_upper() + "_DEFEATED")
