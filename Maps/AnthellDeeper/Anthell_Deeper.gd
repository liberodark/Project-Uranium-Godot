extends Node2D # gen_map.py Map083

var map_name = "Anthell Deeper"
...
var _trainer_data = {
	"Trainer1": {"pre": "MAP083_TRAINER_1", "defeat": "MAP083_TRAINER_1_DEFEAT", "portrait": "Graphics/Characters/trainer045.png"},
	"Trainer2": {"pre": "MAP083_TRAINER_2", "defeat": "MAP083_TRAINER_2_DEFEAT", "portrait": "Graphics/Characters/trainer045.png"},
	"Trainer3": {"pre": "MAP083_TRAINER_3", "defeat": "MAP083_TRAINER_3_DEFEAT", "portrait": "Graphics/Characters/trainer045.png"},
	"Trainer4": {"pre": "MAP083_TRAINER_4", "defeat": "MAP083_TRAINER_4_DEFEAT", "portrait": "Graphics/Characters/trainer045.png"},
	"Trainer5": {"pre": "MAP083_TRAINER_5", "defeat": "MAP083_TRAINER_5_DEFEAT", "portrait": "Graphics/Characters/trainer045.png"}
}

func _ready():
	Global.game.player.connect("trainer_battle", Callable(self, "trainer_battle"))
	for c in $NPC_Layer.get_children():
		if String(c.name).begins_with("Item") and Global.past_events.has("MAP083_ITEM_" + String(c.name).trim_prefix("Item") + "_TAKEN"):
			c.queue_free()
		elif String(c.name).begins_with("Trainer") and Global.past_events.has("MAP083_" + String(c.name).to_upper() + "_DEFEATED"):
			c.seeking = false
			c.defeated = true
...
func trainer_battle(npc_trainer):
	...
	if Global.game.battle.player_won:
		...
		Global.past_events.append("MAP083_" + String(npc_trainer.name).to_upper() + "_DEFEATED")
