extends Node2D # gen_map.py Map191

var map_name = "Nuclear Plant Zeta(3 East)"
...
func interaction(check_pos : Vector2, _direction):
	if check_pos == Vector2(272, 304):
		Global.game.play_dialogue("MAP191_NPC_1")
	if get_node_or_null("NPC_Layer/Item1") != null and check_pos == $NPC_Layer/Item1.position:
		Global.game.lock_player()
		Global.past_events.append("MAP191_ITEM_1_TAKEN")
		Global.inventory.add_item_by_id_multiple(578, 1)
		Global.game.recive_item(578)
...
var _trainer_data = {
	"Trainer1": {"pre": "MAP191_TRAINER_1", "defeat": "MAP191_TRAINER_1_DEFEAT", "portrait": "Graphics/Characters/trainer119.png"}
}

func _ready():
	Global.game.player.connect("trainer_battle", Callable(self, "trainer_battle"))
	for c in $NPC_Layer.get_children():
		if String(c.name).begins_with("Item") and Global.past_events.has("MAP191_ITEM_" + String(c.name).trim_prefix("Item") + "_TAKEN"):
			c.queue_free()
		elif String(c.name).begins_with("Trainer") and Global.past_events.has("MAP191_" + String(c.name).to_upper() + "_DEFEATED"):
			c.seeking = false
			c.defeated = true
...
func trainer_battle(npc_trainer):
	...
	if Global.game.battle.player_won:
		...
		Global.past_events.append("MAP191_" + String(npc_trainer.name).to_upper() + "_DEFEATED")
	Global.game.release_player()
