extends Node2D # gen_map.py Map017

var map_name = "Rochfale Tunnel"
...
var _trainer_data = {
	"Trainer1": {"pre": "MAP017_TRAINER_1", "defeat": "MAP017_TRAINER_1_DEFEAT", "portrait": "Graphics/Characters/trainer004.png"},
	"Trainer2": {"pre": "MAP017_TRAINER_2", "defeat": "MAP017_TRAINER_2_DEFEAT", "portrait": "Graphics/Characters/trainer004.png"},
	"Trainer3": {"pre": "MAP017_TRAINER_3", "defeat": "MAP017_TRAINER_3_DEFEAT", "portrait": "Graphics/Characters/trainer004.png"},
	"Trainer4": {"pre": "MAP017_TRAINER_4", "defeat": "MAP017_TRAINER_4_DEFEAT", "portrait": "Graphics/Characters/trainer055.png"}
}

func _ready():
	Global.game.player.connect("trainer_battle", Callable(self, "trainer_battle"))
	for c in $NPC_Layer.get_children():
		if String(c.name).begins_with("Item") and Global.past_events.has("MAP017_ITEM_" + String(c.name).trim_prefix("Item") + "_TAKEN"):
			c.queue_free()
		elif String(c.name).begins_with("Trainer") and Global.past_events.has("MAP017_" + String(c.name).to_upper() + "_DEFEATED"):
			c.seeking = false
			c.defeated = true
...
func trainer_battle(npc_trainer):
	...
	if Global.game.battle.player_won:
		...
		Global.past_events.append("MAP017_" + String(npc_trainer.name).to_upper() + "_DEFEATED")
