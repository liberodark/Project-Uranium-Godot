extends Node2D # gen_map.py Map007
...
func interaction(check_pos : Vector2, _direction):
	if check_pos == Vector2(368, 1488):
		Global.game.play_dialogue("MAP007_SIGN_1")
	if get_node_or_null("NPC_Layer/Item1") != null and check_pos == $NPC_Layer/Item1.position:
		Global.game.lock_player()
		Global.past_events.append("MAP007_ITEM_1_TAKEN")
	...
	if get_node_or_null("NPC_Layer/Item2") != null and check_pos == $NPC_Layer/Item2.position:
		Global.game.lock_player()
		Global.past_events.append("MAP007_ITEM_2_TAKEN")
	...
	if get_node_or_null("NPC_Layer/Item3") != null and check_pos == $NPC_Layer/Item3.position:
		Global.game.lock_player()
		Global.past_events.append("MAP007_ITEM_3_TAKEN")
		Global.inventory.add_item_by_id_multiple(122, 1)
		Global.game.recive_item(122)
		$NPC_Layer/Item3.queue_free()
		await Global.game.end_of_event
		Global.game.release_player()
	if get_node_or_null("NPC_Layer/Item4") != null and check_pos == $NPC_Layer/Item4.position:
		Global.game.lock_player()
		Global.past_events.append("MAP007_ITEM_4_TAKEN")
		Global.inventory.add_item_by_id_multiple(271, 1)
		Global.game.recive_item(271)
		$NPC_Layer/Item4.queue_free()
		await Global.game.end_of_event
		Global.game.release_player()
	if get_node_or_null("NPC_Layer/Item5") != null and check_pos == $NPC_Layer/Item5.position:
		Global.game.lock_player()
		Global.past_events.append("MAP007_ITEM_5_TAKEN")
		Global.inventory.add_item_by_id_multiple(164, 1)
		Global.game.recive_item(164)
...
var _trainer_data = {
	"Trainer1": {"pre": "MAP007_TRAINER_1", "defeat": "MAP007_TRAINER_1_DEFEAT", "portrait": "Graphics/Characters/trainer056.png"}
}

func _ready():
	Global.game.player.connect("trainer_battle", Callable(self, "trainer_battle"))
	for c in $NPC_Layer.get_children():
		if String(c.name).begins_with("Item") and Global.past_events.has("MAP007_ITEM_" + String(c.name).trim_prefix("Item") + "_TAKEN"):
			c.queue_free()
		elif String(c.name).begins_with("Trainer") and Global.past_events.has("MAP007_" + String(c.name).to_upper() + "_DEFEATED"):
			c.seeking = false
			c.defeated = true
...
func trainer_battle(npc_trainer):
	...
	if Global.game.battle.player_won:
		...
		Global.past_events.append("MAP007_" + String(npc_trainer.name).to_upper() + "_DEFEATED")
	Global.game.release_player()
