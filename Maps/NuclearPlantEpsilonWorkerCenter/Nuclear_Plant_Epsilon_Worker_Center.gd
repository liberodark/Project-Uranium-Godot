extends Node2D # gen_map.py Map125

var map_name = "Nuclear Plant Epsilon(Worker Center)"

func interaction(check_pos : Vector2, _direction):
	if check_pos == Vector2(400, 432):
		Global.game.play_dialogue("MAP125_NPC_1")
	if check_pos == Vector2(496, 240):
		Global.game.play_dialogue("MAP125_NPC_2")
	if check_pos == Vector2(272, 240):
		Global.game.play_dialogue("MAP125_NPC_3")
	if check_pos == Vector2(208, 496):
		_nurse_heal($NPC_Layer/Nurse1)
	if check_pos == Vector2(400, 336):
		Global.game.open_shop(['GREATBALL', 'SUPERPOTION', 'FULLHEAL', 'ESCAPEROPE'])
	return null
...
func _nurse_heal(npc):
	...
	if accepted:
		...
		await Global.game.event_dialogue_end
	Global.game.last_heal_point = "res://Maps/NuclearPlantEpsilon1/Nuclear_Plant_Epsilon_1.tscn"
