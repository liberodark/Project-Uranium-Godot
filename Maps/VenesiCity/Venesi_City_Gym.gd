extends Node2D # gen_map.py Map147

var map_name = "Venesi City(Gym)"

func interaction(check_pos : Vector2, _direction):
	if check_pos == Vector2(496, 1104):
		Global.game.play_dialogue("MAP147_NPC_1")
	return null

var _trainer_data = {
	"Trainer1": {"pre": "MAP147_TRAINER_1", "defeat": "MAP147_TRAINER_1_DEFEAT", "portrait": "Graphics/Characters/trainer100.png"},
	"Trainer2": {"pre": "MAP147_TRAINER_2", "defeat": "MAP147_TRAINER_2_DEFEAT", "portrait": "Graphics/Characters/trainer100.png"},
	"Trainer3": {"pre": "MAP147_TRAINER_3", "defeat": "MAP147_TRAINER_3_DEFEAT", "portrait": "Graphics/Characters/trainer099.png"},
	"Trainer4": {"pre": "MAP147_TRAINER_4", "defeat": "MAP147_TRAINER_4_DEFEAT", "portrait": "Graphics/Characters/trainer099.png"},
	"Trainer5": {"pre": "MAP147_TRAINER_5", "defeat": "MAP147_TRAINER_5_DEFEAT", "portrait": "Graphics/Characters/trainer099.png"},
	"Trainer6": {"pre": "MAP147_TRAINER_6", "defeat": "MAP147_TRAINER_6_DEFEAT", "portrait": "Graphics/Characters/trainer101.png"},
	"Trainer7": {"pre": "MAP147_TRAINER_7", "defeat": "MAP147_TRAINER_7_DEFEAT", "portrait": "Graphics/Characters/trainer102.png"}
}

func _ready():
	Global.game.player.connect("trainer_battle", Callable(self, "trainer_battle"))
	for c in $NPC_Layer.get_children():
		if String(c.name).begins_with("Item") and Global.past_events.has("MAP147_ITEM_" + String(c.name).trim_prefix("Item") + "_TAKEN"):
			c.queue_free()
		elif String(c.name).begins_with("Trainer") and Global.past_events.has("MAP147_" + String(c.name).to_upper() + "_DEFEATED"):
			c.seeking = false
			c.defeated = true
...
func trainer_battle(npc_trainer):
	...
	if Global.game.battle.player_won:
		...
		Global.past_events.append("MAP147_" + String(npc_trainer.name).to_upper() + "_DEFEATED")
