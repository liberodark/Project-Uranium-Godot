extends Node2D # gen_map.py Map069

var map_name = "Amatree Town(Gym)"

func interaction(check_pos : Vector2, _direction):
	if check_pos == Vector2(592, 1008):
		Global.game.play_dialogue("MAP069_NPC_1")
	return null

var _trainer_data = {
	"Trainer1": {"pre": "MAP069_TRAINER_1", "defeat": "MAP069_TRAINER_1_DEFEAT", "portrait": "Graphics/Characters/trainer094.png"},
	"Trainer2": {"pre": "MAP069_TRAINER_2", "defeat": "MAP069_TRAINER_2_DEFEAT", "portrait": "Graphics/Characters/trainer095.png"},
	"Trainer3": {"pre": "MAP069_TRAINER_3", "defeat": "MAP069_TRAINER_3_DEFEAT", "portrait": "Graphics/Characters/trainer095.png"},
	"Trainer4": {"pre": "MAP069_TRAINER_4", "defeat": "MAP069_TRAINER_4_DEFEAT", "portrait": "Graphics/Characters/trainer012.png"},
	"Trainer5": {"pre": "MAP069_TRAINER_5", "defeat": "MAP069_TRAINER_5_DEFEAT", "portrait": "Graphics/Characters/trainer056.png"}
}

func _ready():
	Global.game.player.connect("trainer_battle", Callable(self, "trainer_battle"))
	for c in $NPC_Layer.get_children():
		if String(c.name).begins_with("Item") and Global.past_events.has("MAP069_ITEM_" + String(c.name).trim_prefix("Item") + "_TAKEN"):
			c.queue_free()
		elif String(c.name).begins_with("Trainer") and Global.past_events.has("MAP069_" + String(c.name).to_upper() + "_DEFEATED"):
			c.seeking = false
			c.defeated = true
...
func trainer_battle(npc_trainer):
	...
	if Global.game.battle.player_won:
		...
		Global.past_events.append("MAP069_" + String(npc_trainer.name).to_upper() + "_DEFEATED")
