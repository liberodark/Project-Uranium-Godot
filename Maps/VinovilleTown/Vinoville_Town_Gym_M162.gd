extends Node2D # gen_map.py Map162

var map_name = "Vinoville Town(Gym)"

var _trainer_data = {
	"Trainer1": {"pre": "MAP162_TRAINER_1", "defeat": "MAP162_TRAINER_1_DEFEAT", "portrait": "Graphics/Characters/trainer027.png"},
	"Trainer2": {"pre": "MAP162_TRAINER_2", "defeat": "MAP162_TRAINER_2_DEFEAT", "portrait": "Graphics/Characters/trainer061.png"},
	"Trainer3": {"pre": "MAP162_TRAINER_3", "defeat": "MAP162_TRAINER_3_DEFEAT", "portrait": "Graphics/Characters/trainer090.png"}
}

func _ready():
	Global.game.player.connect("trainer_battle", Callable(self, "trainer_battle"))
	for c in $NPC_Layer.get_children():
		if String(c.name).begins_with("Item") and Global.past_events.has("MAP162_ITEM_" + String(c.name).trim_prefix("Item") + "_TAKEN"):
			c.queue_free()
		elif String(c.name).begins_with("Trainer") and Global.past_events.has("MAP162_" + String(c.name).to_upper() + "_DEFEATED"):
			c.seeking = false
			c.defeated = true
...
func trainer_battle(npc_trainer):
	...
	if Global.game.battle.player_won:
		...
		Global.past_events.append("MAP162_" + String(npc_trainer.name).to_upper() + "_DEFEATED")
