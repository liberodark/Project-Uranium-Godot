extends Node2D # gen_map.py Map148

var map_name = "Vinoville Town(Gym)"

func interaction(check_pos : Vector2, _direction):
	if check_pos == Vector2(816, 1744):
		Global.game.play_dialogue("MAP148_SIGN_1")
	if check_pos == Vector2(816, 1456):
		Global.game.play_dialogue("MAP148_SIGN_2")
	if check_pos == Vector2(624, 1008):
		Global.game.play_dialogue("MAP148_SIGN_3")
	if check_pos == Vector2(1136, 1008):
		Global.game.play_dialogue("MAP148_SIGN_4")
	if check_pos == Vector2(912, 1808):
		Global.game.play_dialogue("MAP148_NPC_1")
	return null

var _trainer_data = {
	"Trainer1": {"pre": "MAP148_TRAINER_1", "defeat": "MAP148_TRAINER_1_DEFEAT", "portrait": "Graphics/Characters/trainer055.png"},
	"Trainer2": {"pre": "MAP148_TRAINER_2", "defeat": "MAP148_TRAINER_2_DEFEAT", "portrait": "Graphics/Characters/trainer027.png"},
	"Trainer3": {"pre": "MAP148_TRAINER_3", "defeat": "MAP148_TRAINER_3_DEFEAT", "portrait": "Graphics/Characters/trainer062.png"},
	"Trainer4": {"pre": "MAP148_TRAINER_4", "defeat": "MAP148_TRAINER_4_DEFEAT", "portrait": "Graphics/Characters/trainer090.png"},
	"Trainer5": {"pre": "MAP148_TRAINER_5", "defeat": "MAP148_TRAINER_5_DEFEAT", "portrait": "Graphics/Characters/trainer092.png"}
}

func _ready():
	Global.game.player.connect("trainer_battle", Callable(self, "trainer_battle"))
	for c in $NPC_Layer.get_children():
		if String(c.name).begins_with("Item") and Global.past_events.has("MAP148_ITEM_" + String(c.name).trim_prefix("Item") + "_TAKEN"):
			c.queue_free()
		elif String(c.name).begins_with("Trainer") and Global.past_events.has("MAP148_" + String(c.name).to_upper() + "_DEFEATED"):
			c.seeking = false
			c.defeated = true
...
func trainer_battle(npc_trainer):
	...
	if Global.game.battle.player_won:
		...
		Global.past_events.append("MAP148_" + String(npc_trainer.name).to_upper() + "_DEFEATED")
