extends Node2D # gen_map.py Map202

var map_name = "Lanthanite Core"
...
func interaction(check_pos : Vector2, _direction):
	if check_pos == Vector2(2000, 848):
		Global.game.play_dialogue("MAP202_NPC_1")
	if check_pos == Vector2(1872, 848):
		_nurse_heal($NPC_Layer/Nurse1)
	if check_pos == Vector2(2000, 752):
		_nurse_heal($NPC_Layer/Nurse2)
	if get_node_or_null("NPC_Layer/Item1") != null and check_pos == $NPC_Layer/Item1.position:
		Global.game.lock_player()
		Global.past_events.append("MAP202_ITEM_1_TAKEN")
		Global.inventory.add_item_by_id_multiple(261, 1)
		Global.game.recive_item(261)
	...
	if get_node_or_null("NPC_Layer/Item2") != null and check_pos == $NPC_Layer/Item2.position:
		Global.game.lock_player()
		Global.past_events.append("MAP202_ITEM_2_TAKEN")
		Global.inventory.add_item_by_id_multiple(514, 1)
		Global.game.recive_item(514)
		$NPC_Layer/Item2.queue_free()
		await Global.game.end_of_event
		Global.game.release_player()
	if get_node_or_null("NPC_Layer/Item3") != null and check_pos == $NPC_Layer/Item3.position:
		Global.game.lock_player()
		Global.past_events.append("MAP202_ITEM_3_TAKEN")
	...
	if get_node_or_null("NPC_Layer/Item4") != null and check_pos == $NPC_Layer/Item4.position:
		Global.game.lock_player()
		Global.past_events.append("MAP202_ITEM_4_TAKEN")
		Global.inventory.add_item_by_id_multiple(591, 1)
		Global.game.recive_item(591)
...
var _trainer_data = {
	"Trainer1": {"pre": "MAP202_TRAINER_1", "defeat": "MAP202_TRAINER_1_DEFEAT", "portrait": "Graphics/Characters/trainer108.png"},
	"Trainer2": {"pre": "MAP202_TRAINER_2", "defeat": "MAP202_TRAINER_2_DEFEAT", "portrait": "Graphics/Characters/trainer105.png"},
	"Trainer3": {"pre": "MAP202_TRAINER_3", "defeat": "MAP202_TRAINER_3_DEFEAT", "portrait": "Graphics/Characters/trainer108.png"},
	"Trainer4": {"pre": "MAP202_TRAINER_4", "defeat": "MAP202_TRAINER_4_DEFEAT", "portrait": "Graphics/Characters/trainer105.png"},
	"Trainer5": {"pre": "MAP202_TRAINER_5", "defeat": "MAP202_TRAINER_5_DEFEAT", "portrait": "Graphics/Characters/trainer108.png"},
	"Trainer6": {"pre": "MAP202_TRAINER_6", "defeat": "MAP202_TRAINER_6_DEFEAT", "portrait": "Graphics/Characters/trainer121.png"}
}

func _ready():
	Global.game.player.connect("trainer_battle", Callable(self, "trainer_battle"))
	for c in $NPC_Layer.get_children():
		if String(c.name).begins_with("Item") and Global.past_events.has("MAP202_ITEM_" + String(c.name).trim_prefix("Item") + "_TAKEN"):
			c.queue_free()
		elif String(c.name).begins_with("Trainer") and Global.past_events.has("MAP202_" + String(c.name).to_upper() + "_DEFEATED"):
			c.seeking = false
			c.defeated = true
...
func trainer_battle(npc_trainer):
	...
	if Global.game.battle.player_won:
		...
		Global.past_events.append("MAP202_" + String(npc_trainer.name).to_upper() + "_DEFEATED")
	Global.game.release_player()
...
func _nurse_heal(npc):
	...
	if accepted:
		...
		await Global.game.event_dialogue_end
	Global.game.last_heal_point = "res://Maps/Caves/Lanthanite_Cave.tscn"
	Global.game.release_player()
