extends Node2D # gen_map.py Map105

var map_name = "Route 06 (Garlikid)"

func interaction(check_pos : Vector2, _direction):
	if get_node_or_null("NPC_Layer/Item1") != null and check_pos == $NPC_Layer/Item1.position:
		Global.game.lock_player()
		Global.past_events.append("MAP105_ITEM_1_TAKEN")
		Global.inventory.add_item_by_id_multiple(273, 1)
		Global.game.recive_item(273)
...
var _trainer_data = {
	"Trainer1": {"pre": "MAP105_TRAINER_1", "defeat": "MAP105_TRAINER_1_DEFEAT", "portrait": "Graphics/Characters/trainer088.png"}
}

func _ready():
	Global.game.player.connect("trainer_battle", Callable(self, "trainer_battle"))
	for c in $NPC_Layer.get_children():
		if String(c.name).begins_with("Item") and Global.past_events.has("MAP105_ITEM_" + String(c.name).trim_prefix("Item") + "_TAKEN"):
			c.queue_free()
		elif String(c.name).begins_with("Trainer") and Global.past_events.has("MAP105_" + String(c.name).to_upper() + "_DEFEATED"):
			c.seeking = false
			c.defeated = true
...
func trainer_battle(npc_trainer):
	...
	if Global.game.battle.player_won:
		...
		Global.past_events.append("MAP105_" + String(npc_trainer.name).to_upper() + "_DEFEATED")
	Global.game.release_player()
