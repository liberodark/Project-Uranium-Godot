extends Node2D # gen_map.py Map099

var map_name = "Route 14"
var map_px_size = Vector2(1216, 3520)
var edge_connections = [["W", "res://Maps/VenesiCity/Venesi_City.tscn", 640, 2608, 1856], ["E", "res://Maps/TsukinamiVillage/Tsukinami_Village.tscn", -1152, 16, 2560]]
...
func interaction(check_pos : Vector2, _direction):
	if check_pos == Vector2(560, 1424):
		Global.game.play_dialogue("MAP099_SIGN_1")
	if check_pos == Vector2(560, 1648):
		Global.game.play_dialogue("MAP099_NPC_1")
	if get_node_or_null("NPC_Layer/Item1") != null and check_pos == $NPC_Layer/Item1.position:
		Global.game.lock_player()
		Global.past_events.append("MAP099_ITEM_1_TAKEN")
		Global.inventory.add_item_by_id_multiple(290, 1)
		Global.game.recive_item(290)
	...
	if get_node_or_null("NPC_Layer/Item2") != null and check_pos == $NPC_Layer/Item2.position:
		Global.game.lock_player()
		Global.past_events.append("MAP099_ITEM_2_TAKEN")
		Global.inventory.add_item_by_id_multiple(316, 1)
		Global.game.recive_item(316)
...
var _trainer_data = {
	"Trainer1": {"pre": "MAP099_TRAINER_1", "defeat": "MAP099_TRAINER_1_DEFEAT", "portrait": "Graphics/Characters/trainer035.png"},
	"Trainer2": {"pre": "MAP099_TRAINER_2", "defeat": "MAP099_TRAINER_2_DEFEAT", "portrait": "Graphics/Characters/trainer011.png"},
	"Trainer3": {"pre": "MAP099_TRAINER_3", "defeat": "MAP099_TRAINER_3_DEFEAT", "portrait": "Graphics/Characters/trainer122.png"}
}

func _ready():
	Global.game.player.connect("trainer_battle", Callable(self, "trainer_battle"))
	for c in $NPC_Layer.get_children():
		if String(c.name).begins_with("Item") and Global.past_events.has("MAP099_ITEM_" + String(c.name).trim_prefix("Item") + "_TAKEN"):
			c.queue_free()
		elif String(c.name).begins_with("Trainer") and Global.past_events.has("MAP099_" + String(c.name).to_upper() + "_DEFEATED"):
			c.seeking = false
			c.defeated = true
...
func trainer_battle(npc_trainer):
	...
	if Global.game.battle.player_won:
		...
		Global.past_events.append("MAP099_" + String(npc_trainer.name).to_upper() + "_DEFEATED")
	Global.game.release_player()
