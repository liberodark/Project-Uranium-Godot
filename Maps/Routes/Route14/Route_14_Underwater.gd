extends Node2D # gen_map.py Map118

var map_name = "Route 14(Underwater)"
...
func interaction(check_pos : Vector2, _direction):
	if get_node_or_null("NPC_Layer/Item1") != null and check_pos == $NPC_Layer/Item1.position:
		...
		Global.past_events.append("MAP118_ITEM_1_TAKEN")
		Global.inventory.add_item_by_id_multiple(287, 1)
		Global.game.recive_item(287)
	...
	if get_node_or_null("NPC_Layer/Item2") != null and check_pos == $NPC_Layer/Item2.position:
		Global.game.lock_player()
		Global.past_events.append("MAP118_ITEM_2_TAKEN")
		Global.inventory.add_item_by_id_multiple(250, 1)
		Global.game.recive_item(250)
		$NPC_Layer/Item2.queue_free()
		await Global.game.end_of_event
		Global.game.release_player()
	if get_node_or_null("NPC_Layer/Item3") != null and check_pos == $NPC_Layer/Item3.position:
		Global.game.lock_player()
		Global.past_events.append("MAP118_ITEM_3_TAKEN")
		Global.inventory.add_item_by_id_multiple(588, 1)
		Global.game.recive_item(588)
...
var _trainer_data = {
	"Trainer1": {"pre": "MAP118_TRAINER_1", "defeat": "MAP118_TRAINER_1_DEFEAT", "portrait": "Graphics/Characters/trainer112.png"},
	"Trainer2": {"pre": "MAP118_TRAINER_2", "defeat": "MAP118_TRAINER_2_DEFEAT", "portrait": "Graphics/Characters/trainer112.png"},
	"Trainer3": {"pre": "MAP118_TRAINER_3", "defeat": "MAP118_TRAINER_3_DEFEAT", "portrait": "Graphics/Characters/trainer112.png"}
}

func _ready():
	Global.game.player.connect("trainer_battle", Callable(self, "trainer_battle"))
	for c in $NPC_Layer.get_children():
		if String(c.name).begins_with("Item") and Global.past_events.has("MAP118_ITEM_" + String(c.name).trim_prefix("Item") + "_TAKEN"):
			c.queue_free()
		elif String(c.name).begins_with("Trainer") and Global.past_events.has("MAP118_" + String(c.name).to_upper() + "_DEFEATED"):
			c.seeking = false
			c.defeated = true
...
func trainer_battle(npc_trainer):
	...
	if Global.game.battle.player_won:
		...
		Global.past_events.append("MAP118_" + String(npc_trainer.name).to_upper() + "_DEFEATED")
	Global.game.release_player()
