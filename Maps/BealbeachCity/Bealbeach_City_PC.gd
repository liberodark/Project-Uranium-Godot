extends Node2D # gen_map.py Map109

var map_name = "Bealbeach City(PC)"

func interaction(check_pos : Vector2, _direction):
	if check_pos == Vector2(400, 400):
		Global.game.play_dialogue("MAP109_NPC_1")
	if check_pos == Vector2(272, 368):
		_nurse_heal($NPC_Layer/Nurse1)
	return null

var _trainer_data = {
	"Trainer1": {"pre": "MAP109_TRAINER_1", "defeat": "MAP109_TRAINER_1_DEFEAT", "portrait": "Graphics/Characters/trainer056.png"}
}

func _ready():
	Global.game.player.connect("trainer_battle", Callable(self, "trainer_battle"))
	for c in $NPC_Layer.get_children():
		if String(c.name).begins_with("Item") and Global.past_events.has("MAP109_ITEM_" + String(c.name).trim_prefix("Item") + "_TAKEN"):
			c.queue_free()
		elif String(c.name).begins_with("Trainer") and Global.past_events.has("MAP109_" + String(c.name).to_upper() + "_DEFEATED"):
			c.seeking = false
			c.defeated = true
...
func trainer_battle(npc_trainer):
	...
	if Global.game.battle.player_won:
		...
		Global.past_events.append("MAP109_" + String(npc_trainer.name).to_upper() + "_DEFEATED")
...
func _nurse_heal(npc):
	...
	if accepted:
		...
		await Global.game.event_dialogue_end
	Global.game.last_heal_point = "res://Maps/BealbeachCity/Bealbeach_City.tscn"
	Global.game.release_player()
