extends Node2D # gen_map.py Map196

var map_name = "Championship Site (gamma)"

var _trainer_data = {
	"Trainer1": {"pre": "MAP196_TRAINER_1", "defeat": "MAP196_TRAINER_1_DEFEAT", "portrait": "Graphics/Characters/trainer120.png"}
}

func _ready():
	Global.game.player.connect("trainer_battle", Callable(self, "trainer_battle"))
	for c in $NPC_Layer.get_children():
		if String(c.name).begins_with("Item") and Global.past_events.has("MAP196_ITEM_" + String(c.name).trim_prefix("Item") + "_TAKEN"):
			c.queue_free()
		elif String(c.name).begins_with("Trainer") and Global.past_events.has("MAP196_" + String(c.name).to_upper() + "_DEFEATED"):
			c.seeking = false
			c.defeated = true
...
func trainer_battle(npc_trainer):
	...
	if Global.game.battle.player_won:
		...
		Global.past_events.append("MAP196_" + String(npc_trainer.name).to_upper() + "_DEFEATED")
