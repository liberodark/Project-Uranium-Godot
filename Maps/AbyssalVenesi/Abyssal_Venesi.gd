extends Node2D # gen_map.py Map071

var map_name = "Abyssal Venesi"
...
func interaction(check_pos : Vector2, _direction):
	if check_pos == Vector2(1776, 2352):
		Global.game.play_dialogue("MAP071_SIGN_1")
	if check_pos == Vector2(2000, 1616):
		Global.game.play_dialogue("MAP071_SIGN_2")
	if check_pos == Vector2(1040, 1520):
		Global.game.play_dialogue("MAP071_SIGN_3")
	if check_pos == Vector2(1232, 1488):
		Global.game.play_dialogue("MAP071_SIGN_4")
	if check_pos == Vector2(944, 2288):
		Global.game.play_dialogue("MAP071_SIGN_5")
	if check_pos == Vector2(1904, 816):
		Global.game.play_dialogue("MAP071_NPC_1")
	if check_pos == Vector2(1008, 2320):
		Global.game.play_dialogue("MAP071_NPC_2")
	if check_pos == Vector2(2000, 1520):
		Global.game.play_dialogue("MAP071_NPC_3")
	if check_pos == Vector2(752, 1520):
		Global.game.play_dialogue("MAP071_NPC_4")
	if check_pos == Vector2(1264, 1520):
		Global.game.play_dialogue("MAP071_NPC_5")
	if check_pos == Vector2(1296, 1520):
		Global.game.play_dialogue("MAP071_NPC_6")
	if check_pos == Vector2(720, 1584):
		Global.game.play_dialogue("MAP071_NPC_7")
	if check_pos == Vector2(656, 1584):
		Global.game.play_dialogue("MAP071_NPC_8")
	if check_pos == Vector2(688, 1552):
		Global.game.play_dialogue("MAP071_NPC_9")
	if check_pos == Vector2(1424, 2288):
		Global.game.play_dialogue("MAP071_NPC_10")
	if check_pos == Vector2(1744, 912):
		_nurse_heal($NPC_Layer/Nurse1)
	return null

var _trainer_data = {
	"Trainer1": {"pre": "MAP071_TRAINER_1", "defeat": "MAP071_TRAINER_1_DEFEAT", "portrait": "Graphics/Characters/trainer127.png"}
}

func _ready():
	Global.game.player.connect("trainer_battle", Callable(self, "trainer_battle"))
	for c in $NPC_Layer.get_children():
		if String(c.name).begins_with("Item") and Global.past_events.has("MAP071_ITEM_" + String(c.name).trim_prefix("Item") + "_TAKEN"):
			c.queue_free()
		elif String(c.name).begins_with("Trainer") and Global.past_events.has("MAP071_" + String(c.name).to_upper() + "_DEFEATED"):
			c.seeking = false
			c.defeated = true
...
func trainer_battle(npc_trainer):
	...
	if Global.game.battle.player_won:
		...
		Global.past_events.append("MAP071_" + String(npc_trainer.name).to_upper() + "_DEFEATED")
