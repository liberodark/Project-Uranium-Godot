...
func _ready():
	...
	$PokeGet/Grid.texture_repeat = CanvasItem.TEXTURE_REPEAT_ENABLED
	$PokeGet/Grid2.texture_repeat = CanvasItem.TEXTURE_REPEAT_ENABLED
	$PokeGet/Light3D.texture_repeat = CanvasItem.TEXTURE_REPEAT_ENABLED
...
func Poke_get_slide(poke : int):
	...
	match poke:
		...
		1:
			...
			var poke_tween = create_tween()
			poke_tween.tween_property($PokeGet/Orchynx, "position", Vector2(246,170), 0.25).set_trans(Tween.TRANS_LINEAR).set_ease(Tween.EASE_IN_OUT)
			await poke_tween.finished
		2:
			orchynx.play("FadeOut")
			electux.play("FadeIn")
			raptorch.play("FadeOut")
			...
			# Godot 4: Create tween programmatically
			var poke_tween = create_tween()
			poke_tween.tween_property($PokeGet/Electux, "position", Vector2(246,170), 0.25).set_trans(Tween.TRANS_LINEAR).set_ease(Tween.EASE_IN_OUT)
			await poke_tween.finished
	# Play music
