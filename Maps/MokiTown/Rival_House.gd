...
var background_music = "res://Audio/BGM/PU-Hero_House.ogg";
...
func event1():
	...
	if !Global.past_events.has(event_name) && Global.past_events.has("EVENT_BAMBOLAB_1_COMPLETE"):
		...
		cameron = load("res://Utilities/NPC.tscn").instantiate()
		...
		if Global.past_events.has("EVENT_BAMBOLAB_1_WIN"):
			...
			var sound = load("res://Audio/ME/Jinlge_-_KeyItem.ogg")
			sound.loop = false
		...
		if Global.past_events.has("EVENT_BAMBOLAB_1_LOSS"):
			...
			await Global.game.dialogue_system.finished_printing

			# Show yesno prompt
			var yesno = load("res://Utilities/UI/YesNoPrompt.tscn").instantiate()
			yesno.set_screen_position(Vector2(350, 150))
			self.add_child(yesno)

			await yesno.selected
